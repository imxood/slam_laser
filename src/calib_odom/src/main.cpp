...
bool Scan2::getOdomPose(Eigen::Vector3d &pose, const ros::Time &t)
{
	...
	try
	{
		// 然后从tf中找出base_frame_相对odom_frame_的Transform, 然后在初始化位置ident的基础上进行Transform(即transform * ident),
		// 最终得到了base_frame_的位置
	...
	// try
	// {
	// 	tf_.lookupTransform(odom_frame_, base_frame_, t, odom_pose);
	// }
...
Eigen::Vector3d cal_delta_distance(Eigen::Vector3d odom_pose)
{
	...
	d_pos =  T.inverse() * (now_pos - last_pos);

	// d_pos << T * (now_pos - last_pos);
