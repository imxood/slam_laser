/**
 * @file /kobuki_driver/src/driver/diff_control.cpp
 *
...
** Implementation
*****************************************************************************/
DiffDrive::DiffDrive(double wheelRadius, double bias, double radPerTick)
	: last_velocity_left(0.0),
	  ...
	  bias(bias),			   // wheelbase, wheel_to_wheel, in [m]
	  wheel_radius(wheelRadius),	// radius of main wheel, in [m]
	  tick_to_rad(radPerTick),
 ...
 */
void DiffDrive::update(const unsigned long &time_stamp,
					   ...
					   ecl::linear_algebra::Vector3d &pose_update_rates)
{
	...
	int curr_tick_left = 0;
	int curr_tick_right = 0;
	...
	printf("tick_to_rad * left_diff_ticks: %lf,  tick_to_rad * right_diff_ticks: %lf\n", tick_to_rad * left_diff_ticks, tick_to_rad * right_diff_ticks);
...
{
	if (value > static_cast<double>(SHRT_MAX))
		return SHRT_MAX;
	if (value < static_cast<double>(SHRT_MIN))
		return SHRT_MIN;
