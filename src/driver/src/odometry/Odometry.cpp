...
Odometry::Odometry() : odom_frame("odom"),
...
void Odometry::init(ros::NodeHandle &nh, const std::string &name)
{
	...
	ROS_INFO_STREAM("Robot : Velocity commands timeout: " << cmd_vel_timeout << " seconds [" << name << "].");

	if (!nh.getParam("odom_frame", odom_frame))
	{
		ROS_WARN_STREAM("Robot : no param server setting for odom_frame, using default [" << odom_frame << "][" << name << "].");
	}
	else
	{
		ROS_INFO_STREAM("Robot : using odom_frame [" << odom_frame << "][" << name << "].");
	}

	if (!nh.getParam("base_frame", base_frame))
	{
		ROS_WARN_STREAM("Robot : no param server setting for base_frame, using default [" << base_frame << "][" << name << "].");
	}
	else
	{
		ROS_INFO_STREAM("Robot : using base_frame [" << base_frame << "][" << name << "].");
	}

	if (!nh.getParam("publish_tf", publish_tf))
	{
		ROS_WARN_STREAM("Robot : no param server setting for publish_tf, using default [" << publish_tf << "][" << name << "].");
	}
	else
	{
		if (publish_tf)
		{
			ROS_INFO_STREAM("Robot : publishing transforms [" << name << "].");
		}
		else
		{
			ROS_INFO_STREAM("Robot : not publishing transforms (see robot_pose_ekf) [" << name << "].");
		}
	}

	if (!nh.getParam("use_imu_heading", use_imu_heading))
	{
		ROS_WARN_STREAM("Robot : no param server setting for use_imu_heading, using default [" << use_imu_heading << "][" << name << "].");
	}
	else
	{
		if (use_imu_heading)
		{
			ROS_INFO_STREAM("Robot : using imu data for heading [" << name << "].");
		}
		else
		{
			ROS_INFO_STREAM("Robot : using encoders for heading (see robot_pose_ekf) [" << name << "].");
		}
	}
...
void Odometry::publishTransform(const geometry_msgs::Quaternion &odom_quat)
{
	...
	//printf("pose.x, pose.y: %f, %f", pose.x(), pose.y());
}
