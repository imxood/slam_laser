...
#ifndef _SXSProtocol_H
#define _SXSProtocol_H
...
#include <robot_driver/servo/SXS_MTP.h>
...
class SXSProtocol
{
  ...
  public:
	...
	u8 End;   //处理器大小端结构
  protected:
	virtual int writeSXS(unsigned char *nDat, int nLen) = 0;
	virtual int readSXS(unsigned char *nDat, int nLen) = 0;
	virtual int writeSXS(unsigned char bDat) = 0;
	virtual void flushSXS() = 0;

  private:
	void writeBuf(u8 ID, u8 MemAddr, u8 *nDat, u8 nLen, u8 Fun);
	int writePos(u8 ID, u16 Position, u16 Time, u16 Speed, u8 Fun);
	void Host2SXS(u8 *DataL, u8 *DataH, int Data); //1个16位数拆分为2个8位数
	int SXS2Host(u8 DataL, u8 DataH);			   //2个8位数组合为1个16位数
	int Ack(u8 ID);								   //返回应答
